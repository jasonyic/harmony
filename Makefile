...
export LD_LIBRARY_PATH:=$(TOP)/bls/lib:$(TOP)/mcl/lib:/usr/local/opt/openssl/lib:/opt/homebrew/opt/gmp/lib/:/opt/homebrew/opt/openssl/lib:/usr/lib
...
build-docker: linux_static
	docker buildx build --platform=$(GOOS)/$(GOARCH) --pull -t harmonyone/harmony:$(HARMONY_VERSION) \
	-t harmonyone/harmony:latest \
	-f scripts/docker/Dockerfile . --output=type=docker

test-docker-rpc: build-docker
	bash ./test/docker-rpc.sh run
